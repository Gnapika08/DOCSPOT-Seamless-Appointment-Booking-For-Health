...
const DoctorAppointment = () => {
  ...
  const { data, isLoading, isSuccess } = useDoctorAppointmentsQuery({ userId });

  const [appointmentStatus, { isLoading: appointmentLoading }] =
    useAppointmentStatusMutation();

  const appointmentHandler = async (id, status) => {
    try {
      const payload = {
        appointmentId: id,
        status,
      ...
      const appointment = await appointmentStatus(payload);
